...
When using `r values_to` almost always with a long transformation youll get big values in the visit/time variable/column transformation because you're getting these time values into a string. So, use `names_prefix` if there is an easy pattern to remove the bulk. Check what visit column values look like before and after this step by running only parts of the code at a time.
...
Because they have the same variables we can easily stack these dfs on top of one another using `r bind_rows`

```{r}
lotr_df = 
  bind_rows(fellowship_rings, two_towers, Return_of_king) |> 
...
```
Note: `r separate()` takes the variable you want to separate, [ into = ] by writing the names of the new variables you are creating) and then [sep = ] how you are separating (by what delimiter or index you want to split by). 
...
Interesting join: 
`r anti_join()` identifies which observations are not in the left hand side function. It allows you to figure out the differences between the two datasets. (It tells you what observations are not in the right hand side from the left hand side)
