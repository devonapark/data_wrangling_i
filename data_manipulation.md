...
Select by prefix (`starts_with` is what he uses most often, but there
also `ends_with` or `contains`)
...
What comes first in the command is what is going to come first in the
output.
...
Theoretically you can do something like `litters_df$group` which pulls
the variable out of the dataframe. But this is a clunky, BAD FORM OF
CODING
